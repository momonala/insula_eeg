...
class Gui_Manager {
  ...
  int bgColorGraphs = background_color;
  int gridColor = 200;
  int borderColor = 10;
  int axisColor = 200;
  int fontColor = 1;
  ...
  Gui_Manager(PApplet parent,int win_x, int win_y,int nchan,float displayTime_sec, float default_yScale_uV, 
    ...
    cc = new ChannelController(x_cc, y_cc, w_cc, h_cc, axes_x/2, axes_y/2);
    ...
    //=====================================================================================making the FFT Plot take up the whole screen
  ...
  public void setupMontagePlot(Graph2D g, int win_x, int win_y, float[] axis_relPos,float displayTime_sec, PlotFontInfo fontInfo,String filterDescription) {
    ...
    gbMontage.setGridColour(240, 240, 240, 240, 240, 240);
  ...
  public void draw() {
      ...
      if(graph_mode) //============hiding the gmontage
      {
          gMontage.draw(); 
      }
   ...
   // stopButton.draw();
    ...
    if(!channels_set && !hide_channel_controller)
    {
